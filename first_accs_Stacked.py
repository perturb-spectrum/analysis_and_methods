...
parser.add_argument('--model-type', default='Stacked', type=str, help=' <empty> or Stacked')
...
for epsilon in epsilonz:
    ...
    print(f'Model {args.model_type}Trained with strength {epsilon}/255 has clean acc: {clean_acc_test}')


    pgd_20_acc_test = pgd_eval_for_epsilon(model, val_loader, device, epsilon_test, args)
    print(f'Model {args.model_type}Trained with strength {epsilon}/255 has PGD Adv acc on {epsilon_test}: {pgd_20_acc_test}')
