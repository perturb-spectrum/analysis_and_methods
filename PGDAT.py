...
from utils import clean_eval, pgd_eval, get_timestamp, pgd_eval_for_epsilon
import argparse
...
parser.add_argument('--num-epochs', default=100, type=int, help='num epochs')
parser.add_argument('--epsilon', default=8, type=float, help='perturbation budget = epsilon / 255')
parser.add_argument('--epsilon-test', default=2, type=float, help='attack eval budget = epsilon_test / 255')
...
atk = PGD(model, eps=args.epsilon/255.0, alpha=2/255 * args.epsilon/8, steps=10, random_start=True) 
...
for epoch in range(1, args.num_epochs+1):
    ...
    if (epoch % save_epochs == 0) or (epoch == args.num_epochs) or (epoch == 76): # 76 is args.num_epochs in TRADES github
        val_acc = clean_eval(model, val_loader, device, args)
        pgd20_adv_acc = pgd_eval_for_epsilon(model, val_loader, device, args.epsilon_test, args)
        ...
        os.makedirs(f'saved_state_dicts/PGDAT_{args.model_name}_{args.epsilon}', exist_ok=True)
        torch.save(model.state_dict(), f'saved_state_dicts/PGDAT_{args.model_name}_{args.epsilon}/ep_{epoch}.pth')
    else:
        val_acc = clean_eval(model, val_loader, device, args)
